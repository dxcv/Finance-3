...
##############################################################
# 计算项目投资 IRR 和资本金 IRR
price = list(Price.values())  # 各省电价信息
place = list(Price.keys())  # 对电价相对应的各省份
finance = Finance()
...
for index in range(len(price)):
    ...
    write_excel([pre_pro_irr,after_pro_irr,cap_irr], sheet_name=sheet_name, 
                row_header=row_name, column_header=column_name,file='光伏收益-' 
                + place[index] + '-' + str(price[index]) + '.xlsx')